...
function runTests() {
  ...
  if (test('displays footer with attribution', () => {
    ...
    const combined = logs.join('\n');
    assert.ok(combined.includes('Claude Fulcrum'), 'Should include project name');
  })) passed++; else failed++;
  ...
  if (test('does not crash with confidence > 1.0 (percent > 100)', () => {
    const output = new SkillCreateOutput('repo');
    // confidence 1.5 => percent 150 â€” previously crashed with RangeError
    const logs = captureLog(() => output.patterns([
  ...
  // â”€â”€ Round 27: box and progressBar edge cases â”€â”€
  ...
  if (test('patterns renders negative confidence without crash', () => {
    const output = new SkillCreateOutput('repo');
    // confidence -0.1 => percent -10 â€” Math.max(0, ...) should clamp filled to 0
    const logs = captureLog(() => output.patterns([
  ...
  // â”€â”€ Round 34: header width alignment â”€â”€
  ...
  if (test('header subtitle has correct content area width of 64 chars', () => {
    ...
    const subtitleLine = lines.find(l => l.includes('Extracting patterns'));
    assert.ok(subtitleLine, 'Should find subtitle line');
    // Content between â•‘ and â•‘ should be 64 chars (border is 66 total)
    // Format: â•‘ + content(64) + â•‘ = 66
  ...
  // â”€â”€ Round 35: box() width accuracy â”€â”€
  ...
  if (test('box lines in instincts() match the default box width of 60', () => {
    ...
    // The box() default width is 60 â€” each line should be exactly 60 chars
  ...
  if (test('box lines with custom width match the requested width', () => {
    ...
    // regardless of this.width â€” verify self-consistency at least
  ...
  // â”€â”€ Round 54: analysisResults with zero values â”€â”€
  ...
  // â”€â”€ Round 68: demo function export â”€â”€
  ...
  // â”€â”€ Round 85: patterns() confidence=0 uses ?? (not ||) â”€â”€
  ...
  if (test('patterns() with confidence=0 shows 0%, not 80% (nullish coalescing fix)', () => {
    ...
    const combined = stripAnsi(logs.join('\n'));
    // With ?? operator: 0 ?? 0.8 = 0 â†’ Math.round(0 * 100) = 0 â†’ shows "0%"
    // With || operator (bug): 0 || 0.8 = 0.8 â†’ shows "80%"
    assert.ok(combined.includes('0%'), 'Should show 0% for zero confidence');
    assert.ok(!combined.includes('80%'),
      'Should NOT show 80% â€” confidence=0 is explicitly provided, not missing');
  })) passed++; else failed++;

  // â”€â”€ Round 87: analyzePhase() async method (untested) â”€â”€
  ...
  if (test('analyzePhase completes without error and writes to stdout', () => {
    ...
    try {
      // Call synchronously by accessing the returned promise â€” we just need to
