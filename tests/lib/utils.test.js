...
function runTests() {
  ...
  if (test('isGitRepo returns true in a git repo', () => {
    // We're running from within the Claude Fulcrum repo, so this should be true
  ...
  // â”€â”€ runCommand security: allowlist and metacharacter blocking â”€â”€
  ...
  if (test('runCommand blocks prefix without trailing space', () => {
    // "gitconfig" starts with "git" but not "git " â€” must be blocked
