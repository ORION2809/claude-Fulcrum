...
\`\`\``;
    ...
    assert.strictEqual(meta.project, 'claude-fulcrum');
...
`;
  ...
  if (test('multiline content ending with .tmp is treated as content', () => {
    ...
    const stats = sessionManager.getSessionStats(content);
    // Has newlines so looksLikePath is false â†’ treated as content
  ...
  // â”€â”€ Round 43: getSessionById default excludes content â”€â”€
  ...
  // â”€â”€ Round 54: search filter scope and getSessionPath utility â”€â”€
  ...
  // â”€â”€ Round 66: getSessionById noIdMatch path (date-only string for old format) â”€â”€
  console.log('\nRound 66: getSessionById (noIdMatch â€” date-only match for old format):');
  ...
  if (test('getSessionById finds old-format session by date-only string (noIdMatch)', () => {
    ...
    // Calling with '2026-02-10' â†’ filenameMatch fails (filename !== '2026-02-10' and !== '2026-02-10.tmp')
  ...
  // â”€â”€ Round 30: datetime local-time fix and parseSessionFilename edge cases â”€â”€
  ...
  if (test('datetime matches for January 1 (timezone-sensitive date)', () => {
    // Jan 1 at UTC midnight is Dec 31 in negative offsets â€” this tests the fix
  ...
  if (test('handles old-format filename without session ID', () => {
    // The regex match[2] is undefined for old format â†’ shortId defaults to 'no-id'
    ...
    // Either null (regex doesn't match) or has no-id â€” both are acceptable
    assert.ok(true, 'Old format handled without crash');
  })) passed++; else failed++;

  // â”€â”€ Round 33: birthtime / createdTime fallback â”€â”€
  ...
  if (test('getAllSessions returns createdTime from birthtime when available', () => {
    ...
    // birthtime should be populated on macOS/Windows â€” createdTime should match it
  ...
  // â”€â”€ Round 46: path heuristic and checklist edge cases â”€â”€
  ...
  if (test('recognises Windows drive-letter path as a file path', () => {
    ...
    // (getSessionContent returns null â†’ parseSessionMetadata(null) â†’ defaults)
  ...
  if (test('does not treat bare drive letter without slash as path', () => {
    // "C:session.tmp" has no slash after colon â†’ regex fails â†’ treated as content
  ...
  if (test('uppercase [X] does not match completed items regex', () => {
    const content = '# Test\n\n### Completed\n- [X] Uppercase task\n- [x] Lowercase task\n';
    const meta = sessionManager.parseSessionMetadata(content);
    // Regex is /- \[x\]\s*(.+)/g â€” only matches lowercase [x]
  ...
  // â”€â”€ Round 69: getSessionById returns null when sessions dir missing â”€â”€
  ...
  // â”€â”€ Round 78: getSessionStats reads real file when given existing .tmp path â”€â”€
  console.log('\nRound 78: getSessionStats (actual file path â†’ reads from disk):');
  ...
  if (test('getSessionStats reads from disk when given path to existing .tmp file', () => {
    ...
    try {
      ...
      // Pass the FILE PATH (not content) â€” this exercises looksLikePath branch
  ...
  // â”€â”€ Round 78: getAllSessions hasContent field â”€â”€
  ...
  // â”€â”€ Round 75: deleteSession catch â€” unlinkSync throws on read-only dir â”€â”€
  ...
  // â”€â”€ Round 81: getSessionStats(null) â”€â”€
  ...
  if (test('getSessionStats(null) returns zero lineCount and empty metadata', () => {
    ...
    // typeof null === 'string' is false â†’ looksLikePath = false â†’ content = null.
    // Line 177: content ? content.split('\n').length : 0 â†’ lineCount: 0.
    // parseSessionMetadata(null) returns defaults â†’ totalItems/completedItems/inProgressItems = 0.
  ...
  // â”€â”€ Round 83: getAllSessions TOCTOU statSync catch (broken symlink) â”€â”€
  console.log('\nRound 83: getAllSessions (broken symlink â€” statSync catch):');
  ...
  if (test('getAllSessions skips broken symlink .tmp files gracefully', () => {
    ...
    try {
      ...
      if (error && error.code === 'EPERM') {
        console.log('    (skipped â€” broken symlink creation not permitted on this Windows setup)');
        return;
      }
      throw error;
    }
  ...
  // â”€â”€ Round 84: getSessionById TOCTOU â€” statSync catch returns null for broken symlink â”€â”€
  console.log('\nRound 84: getSessionById (broken symlink â€” statSync catch):');
  ...
  if (test('getSessionById returns null when matching session is a broken symlink', () => {
    ...
    try {
      ...
      // Search by the short ID "deadbeef" â€” should match the broken symlink
  ...
  // â”€â”€ Round 88: parseSessionMetadata null date/started/lastUpdated fields â”€â”€
  ...
  // â”€â”€ Round 89: getAllSessions skips subdirectories (!entry.isFile()) â”€â”€
  ...
  if (test('getAllSessions skips subdirectories inside sessions dir', () => {
    ...
    // This test creates a DIRECTORY â€” entry.isFile() returns false, so it should be skipped.
    ...
    // Create a subdirectory inside sessions dir â€” should be skipped by !entry.isFile()
    const subdir = path.join(sessionsDir, 'some-nested-dir');
    fs.mkdirSync(subdir);

    // Also create a subdirectory whose name ends in .tmp â€” still not a file
  ...
  // â”€â”€ Round 91: getSessionStats with mixed Windows path separators â”€â”€
  ...
  if (test('getSessionStats treats mixed Windows separators as a file path', () => {
    ...
    assert.strictEqual(stats.lineCount, 0,
      'Mixed separators should be treated as path (file does not exist â†’ lineCount 0)');
    assert.strictEqual(stats.totalItems, 0, 'Non-existent path should have 0 items');
  })) passed++; else failed++;

  // â”€â”€ Round 92: getSessionStats with UNC path treated as content â”€â”€
  ...
  // â”€â”€ Round 93: getSessionStats with drive letter but no slash (regex boundary) â”€â”€
  console.log('\nRound 93: getSessionStats (drive letter without slash â€” regex boundary):');
  ...
  // â”€â”€ Round 95: getAllSessions with both negative offset AND negative limit â”€â”€
  ...
  if (test('getAllSessions clamps both negative offset (to 0) and negative limit (to 1) simultaneously', () => {
    ...
    // offset clamped: Math.max(0, Math.floor(-5)) â†’ 0
    // limit clamped: Math.max(1, Math.floor(-10)) â†’ 1
    // slice(0, 0+1) â†’ first session only
    assert.strictEqual(result.offset, 0,
  ...
  // â”€â”€ Round 96: parseSessionFilename with Feb 30 (impossible date) â”€â”€
  console.log('\nRound 96: parseSessionFilename (Feb 30 â€” impossible date):');
  ...
  if (test('parseSessionFilename rejects Feb 30 (passes day<=31 but fails Date rollover)', () => {
    ...
    // but new Date(2026, 1, 30) â†’ March 2 (rollover), so getMonth() !== 1 â†’ returns null
  ...
  // â”€â”€ Round 96: getAllSessions with limit: Infinity â”€â”€
  console.log('\nRound 96: getAllSessions (limit: Infinity â€” pagination bypass):');
  ...
  // â”€â”€ Round 96: getAllSessions with limit: null â”€â”€
  console.log('\nRound 96: getAllSessions (limit: null â€” destructuring default bypass):');
  ...
  // â”€â”€ Round 97: getAllSessions with whitespace search filters out everything â”€â”€
  console.log('\nRound 97: getAllSessions (whitespace search â€” truthy but unmatched):');
  ...
  // â”€â”€ Round 98: getSessionById with null sessionId throws TypeError â”€â”€
  console.log('\nRound 98: getSessionById (null sessionId â€” crashes at line 297):');

  if (test('getSessionById(null) throws TypeError when session files exist', () => {
    // session-manager.js line 297: `sessionId.length > 0` â€” calling .length on null
  ...
  // â”€â”€ Round 98: parseSessionFilename with null input throws TypeError â”€â”€
  console.log('\nRound 98: parseSessionFilename (null input â€” crashes at line 30):');
  ...
  // â”€â”€ Round 99: writeSessionContent with null path returns false (error caught) â”€â”€
  console.log('\nRound 99: writeSessionContent (null path â€” error handling):');
  ...
  // â”€â”€ Round 100: parseSessionMetadata with ### inside item text (premature section termination) â”€â”€
  console.log('\nRound 100: parseSessionMetadata (### in item text â€” lazy regex truncation):');
...
`;
    ...
    assert.strictEqual(meta.completed.length, 1,
      'Only 1 item extracted â€” second item is after the inner ### terminator');
    assert.strictEqual(meta.completed[0], 'Fix issue',
      'Item text truncated at embedded ### (lazy regex stops at first ### match)');
  })) passed++; else failed++;

  // â”€â”€ Round 101: getSessionStats with non-string input (number) throws TypeError â”€â”€
  console.log('\nRound 101: getSessionStats (non-string input â€” type confusion crash):');
  if (test('getSessionStats(123) throws TypeError (number reaches parseSessionMetadata â†’ .match() fails)', () => {
    // typeof 123 === 'number' â†’ looksLikePath = false â†’ content = 123
    // parseSessionMetadata(123) â†’ !123 is false â†’ 123.match(...) â†’ TypeError
    assert.throws(
  ...
  // â”€â”€ Round 101: appendSessionContent(null, 'content') returns false (error caught) â”€â”€
  console.log('\nRound 101: appendSessionContent (null path â€” error handling):');
  ...
  // â”€â”€ Round 102: getSessionStats with Unix nonexistent .tmp path (looksLikePath heuristic) â”€â”€
  console.log('\nRound 102: getSessionStats (Unix nonexistent .tmp path â€” looksLikePath â†’ null content):');
  if (test('getSessionStats returns zeroed stats when Unix path looks like file but does not exist', () => {
    ...
    // the file-read branch â†’ readFile returns null â†’ parseSessionMetadata(null) returns
    // default empty metadata â†’ lineCount: null ? ... : 0 === 0.
  ...
  // â”€â”€ Round 102: parseSessionMetadata with [x] checked items in In Progress section â”€â”€
  console.log('\nRound 102: parseSessionMetadata ([x] items in In Progress â€” regex skips checked):');
  if (test('parseSessionMetadata skips [x] checked items in In Progress section (regex only matches [ ])', () => {
    ...
    // section are silently ignored â€” they don't match the regex pattern.
    const content = `# Session
...
`;
  ...
  // â”€â”€ Round 104: parseSessionMetadata with whitespace-only notes section â”€â”€
  console.log('\nRound 104: parseSessionMetadata (whitespace-only notes â€” trim reduces to empty):');
  if (test('parseSessionMetadata treats whitespace-only notes as absent (trim â†’ empty string â†’ falsy)', () => {
    // session-manager.js line 139: `metadata.notes = notesSection[1].trim()` â€” when the
    // Notes section heading exists but only contains whitespace/newlines, trim() returns "".
    // Then getSessionStats line 178: `hasNotes: !!metadata.notes` â€” `!!""` is `false`.
...
`;
  ...
  // â”€â”€ Round 105: parseSessionMetadata blank-line boundary truncates section items â”€â”€
  console.log('\nRound 105: parseSessionMetadata (blank line inside section â€” regex stops at \\n\\n):');
  ...
  if (test('parseSessionMetadata drops completed items after a blank line within the section', () => {
    ...
    assert.strictEqual(meta.completed.length, 1,
      'Only Task A captured â€” blank line terminates the section regex before Task B');
    assert.strictEqual(meta.completed[0], 'Task A',
      'First completed item should be Task A');
    // Task B is lost â€” it appears after the blank line, outside the captured range
    assert.strictEqual(meta.inProgress.length, 1,
  ...
  // â”€â”€ Round 106: getAllSessions with array/object limit â€” Number() coercion edge cases â”€â”€
  console.log('\nRound 106: getAllSessions (array/object limit coercion â€” Number([5])â†’5, Number({})â†’NaNâ†’50):');
  if (test('getAllSessions coerces array/object limit via Number() with NaN fallback to 50', () => {
    ...
    try {
      ...
      // Multi-element array: Number([1,2]) â†’ NaN â†’ fallback to 50
      const multiArrResult = freshManager.getAllSessions({ limit: [1, 2] });
      assert.strictEqual(multiArrResult.limit, 50,
        'Multi-element array [1,2] coerces to NaN â†’ fallback to 50');
    } finally {
      process.env.HOME = origHome;
      process.env.USERPROFILE = origUserProfile;
      delete require.cache[require.resolve('../../scripts/lib/session-manager')];
  ...
  // â”€â”€ Round 109: getAllSessions skips .tmp files that don't match session filename format â”€â”€
  console.log('\nRound 109: getAllSessions (non-session .tmp files â€” parseSessionFilename returns null â†’ skip):');
  ...
  // â”€â”€ Round 108: getSessionSize exact boundary at 1024 bytes â€” Bâ†’KB transition â”€â”€
  console.log('\nRound 108: getSessionSize (exact 1024-byte boundary â€” < means 1024 is KB, 1023 is B):');
  if (test('getSessionSize returns KB at exactly 1024 bytes and B at 1023', () => {
    const dir = createTempSessionDir();
    try {
      // Exactly 1024 bytes â†’ size < 1024 is FALSE â†’ goes to KB branch
      ...
      // 1023 bytes â†’ size < 1024 is TRUE â†’ stays in B branch
      ...
      // Exactly 1MB boundary â†’ 1048576 bytes
  ...
  // â”€â”€ Round 110: parseSessionFilename year 0000 â€” JS Date maps year 0 to 1900 â”€â”€
  console.log('\nRound 110: parseSessionFilename (year 0000 â€” Date constructor maps 0â†’1900):');
  if (test('parseSessionFilename with year 0000 produces datetime in 1900 due to JS Date legacy mapping', () => {
    ...
    // So new Date(0, 0, 1) â†’ January 1, 1900 (not year 0000)
    ...
    // Year 100 does NOT get the 1900 mapping â€” it stays as year 100
    ...
    assert.strictEqual(result100.datetime.getFullYear(), 100,
      'Year 100+ is not affected by the 0-99 â†’ 1900-1999 mapping');
  })) passed++; else failed++;

  // â”€â”€ Round 110: parseSessionFilename accepts mixed-case IDs â”€â”€
  ...
  // â”€â”€ Round 111: parseSessionMetadata context with nested triple backticks â€” lazy regex truncation â”€â”€
  console.log('\nRound 111: parseSessionMetadata (nested ``` in context â€” lazy \\S*? stops at first ```):");');
  if (test('parseSessionMetadata context capture truncated by nested triple backticks', () => {
    ...
    // FIRST ``` it encounters â€” even if that's inside the code block content.
    const content = [
      '# Session',
      '',
      '### Context to Load',
  ...
  // â”€â”€ Round 112: getSessionStats with newline-containing absolute path â€” treated as content â”€â”€
  ...
  // â”€â”€ Round 112: appendSessionContent with read-only file â€” returns false â”€â”€
  ...
  if (test('appendSessionContent returns false when file is read-only (EACCES)', () => {
    if (process.platform === 'win32') {
      // chmod doesn't work reliably on Windows â€” skip
  ...
  // â”€â”€ Round 113: parseSessionFilename century leap year validation (1900, 2100 not leap; 2000 is) â”€â”€
  console.log('\nRound 113: parseSessionFilename (century leap year â€” 100/400 rules):');
  if (test('parseSessionFilename rejects Feb 29 in century non-leap years (1900, 2100) but accepts 2000', () => {
    // Gregorian rule: divisible by 100 â†’ NOT leap, UNLESS also divisible by 400
    // 1900: divisible by 100 but NOT by 400 â†’ NOT leap â†’ Feb 29 invalid
    const result1900 = sessionManager.parseSessionFilename('1900-02-29-abcd1234-session.tmp');
    assert.strictEqual(result1900, null,
      '1900 is NOT a leap year (div by 100 but not 400) â€” Feb 29 should be rejected');

    // 2100: same rule â€” NOT leap
    const result2100 = sessionManager.parseSessionFilename('2100-02-29-test1234-session.tmp');
    assert.strictEqual(result2100, null,
      '2100 is NOT a leap year â€” Feb 29 should be rejected');

    // 2000: divisible by 400 â†’ IS leap â†’ Feb 29 valid
    ...
    assert.notStrictEqual(result2000, null,
      '2000 IS a leap year (div by 400) â€” Feb 29 should be accepted');
    assert.strictEqual(result2000.date, '2000-02-29');

    // 2400: also divisible by 400 â†’ IS leap
    ...
    assert.notStrictEqual(result2400, null,
      '2400 IS a leap year (div by 400) â€” Feb 29 should be accepted');
  ...
  // â”€â”€ Round 113: parseSessionMetadata title with markdown formatting â€” raw markdown preserved â”€â”€
  console.log('\nRound 113: parseSessionMetadata (title with markdown formatting â€” raw markdown preserved):');
  ...
  // â”€â”€ Round 115: parseSessionMetadata with CRLF line endings â€” section boundaries differ â”€â”€
  console.log('\nRound 115: parseSessionMetadata (CRLF line endings â€” \\r\\n vs \\n in section regexes):');
  if (test('parseSessionMetadata handles CRLF content â€” title trimmed, sections may over-capture', () => {
    ...
    // In Progress section: \n\n boundary fails on \r\n\r\n, so the lazy [\s\S]*?
    // stops at ### instead â€” this still works because ### is present
    ...
    // Edge case: CRLF content with NO section headers after Completed â€”
  ...
  // â”€â”€ Round 117: getSessionSize boundary values â€” B/KB/MB formatting thresholds â”€â”€
  console.log('\nRound 117: getSessionSize (B/KB/MB formatting at exact boundary thresholds):');
  if (test('getSessionSize formats correctly at Bâ†’KB boundary (1023â†’"1023 B", 1024â†’"1.0 KB") and KBâ†’MB', () => {
    ...
    try {
      ...
      // 1023 bytes â€” last value in B range (size < 1024)
      ...
      // 1024 bytes â€” first value in KB range (size >= 1024, < 1024*1024)
  ...
  // â”€â”€ Round 117: parseSessionFilename accepts uppercase, underscores, and short IDs â”€â”€
  ...
  // â”€â”€ Round 119: parseSessionMetadata "Context to Load" code block extraction â”€â”€
  console.log('\nRound 119: parseSessionMetadata ("Context to Load" â€” code block extraction edge cases):');
  if (test('parseSessionMetadata extracts Context to Load from code block, handles missing/nested blocks', () => {
    ...
    // Missing closing backticks â€” regex doesn't match, context stays empty
    const noClose = [
      '# Session\n\n',
      '### Context to Load\n',
      '```\n',
    ...
    // No code block after header â€” just plain text
    const noBlock = [
      '# Session\n\n',
      '### Context to Load\n',
      'file1.js\n',
    ...
    // Nested code block â€” lazy [\s\S]*? stops at first ```
    const nested = [
      '# Session\n\n',
      '### Context to Load\n',
      '```\n',
  ...
  // â”€â”€ Round 120: parseSessionMetadata "Notes for Next Session" extraction edge cases â”€â”€
  console.log('\nRound 120: parseSessionMetadata ("Notes for Next Session" â€” extraction edge cases):');
  if (test('parseSessionMetadata extracts notes section â€” last section, empty, followed by ###', () => {
  ...
  // â”€â”€ Round 121: parseSessionMetadata Started/Last Updated time extraction â”€â”€
  ...
  if (test('parseSessionMetadata extracts Started and Last Updated times from markdown', () => {
    ...
    assert.strictEqual(neitherMeta.started, null, 'No Started in content â†’ null');
    assert.strictEqual(neitherMeta.lastUpdated, null, 'No Last Updated in content â†’ null');
  ...
  // â”€â”€ Round 122: getSessionById old format (no-id) â€” noIdMatch path â”€â”€
  console.log('\nRound 122: getSessionById (old format no-id â€” date-only filename match):');
  if (test('getSessionById matches old format YYYY-MM-DD-session.tmp via noIdMatch path', () => {
    ...
    try {
      ...
      // Search by date â€” triggers noIdMatch path
      ...
      // Search by non-matching date â€” should not find
  ...
  // â”€â”€ Round 123: parseSessionMetadata with CRLF line endings â€” section boundaries break â”€â”€
  console.log('\nRound 123: parseSessionMetadata (CRLF section boundaries â€” \\n\\n fails to match \\r\\n\\r\\n):');
  if (test('parseSessionMetadata CRLF content: \\n\\n boundary fails, lazy match bleeds across sections', () => {
    ...
    // so section headers still match â€” only blank-line boundaries fail.

    // Test 1: CRLF with ### delimiter â€” works because ### is an alternation
    const crlfWithHash = [
      '# Session Title\r\n',
      '\r\n',
      '### Completed\r\n',
    ...
    // ### delimiter still works â€” lazy match stops at ### In Progress
    ...
    // Test 2: CRLF with \n\n (blank line) delimiter â€” this is where it breaks
    const crlfBlankLine = [
      '# Session\r\n',
      '\r\n',
      '### Completed\r\n',
    ...
    // Test 3: LF version of same content â€” proves \n\n works normally
    ...
    // Test 4: CRLF notes section â€” lazy match goes to $ when \n\n fails
    const crlfNotes = [
      '# Session\r\n',
      '\r\n',
      '### Notes for Next Session\r\n',
    ...
    // On CRLF, \n\n fails â†’ lazy match extends to $ â†’ includes "This should be separate"
    // On LF, \n\n works â†’ notes = "Remember to review" only
  ...
  // â”€â”€ Round 124: getAllSessions with invalid date format (strict equality, no normalization) â”€â”€
  console.log('\nRound 124: getAllSessions (invalid date format â€” strict !== comparison):');
  if (test('getAllSessions date filter uses strict equality so wrong format returns empty', () => {
    // session-manager.js line 228: `if (date && metadata.date !== date)` â€” strict inequality.
    ...
    try {
      ...
      // Correct format â€” should find 1 session
      ...
      // Wrong separator â€” strict !== means no match
      ...
      // null date â€” skips filter, returns all
  ...
  // â”€â”€ Round 124: parseSessionMetadata title edge cases (no space, wrong level, multiple, empty) â”€â”€
  console.log('\nRound 124: parseSessionMetadata (title regex edge cases â€” /^#\\s+(.+)$/m):');
  if (test('parseSessionMetadata title: no space after # fails, ## fails, multiple picks first, empty trims', () => {
    ...
    // No space after # â€” \s+ fails to match
    const noSpace = '#NoSpaceTitle\n\nSome content';
    const meta1 = sessionManager.parseSessionMetadata(noSpace);
    assert.strictEqual(meta1.title, null,
      '#NoSpaceTitle has no whitespace after # â†’ title is null');

    // ## (H2) heading â€” ^ anchors to line start, but # matches first char only
    // /^#\s+/ matches the first # then \s+ would need whitespace, but ## has another #
    // Actually: /^#\s+(.+)$/ â†’ "##" â†’ # then \s+ â†’ # is not whitespace â†’ no match
    const h2 = '## Subtitle\n\nContent';
    const meta2 = sessionManager.parseSessionMetadata(h2);
    assert.strictEqual(meta2.title, null,
      '## heading does not match /^#\\s+/ because second # is not whitespace');

    // Multiple # headings â€” first match wins (regex .match returns first)
    ...
    // # followed by spaces then text â€” leading spaces in capture are trimmed
    ...
    assert.strictEqual(meta5.title, 'Content',
      'Spaces-only after # â†’ \\s+ greedily matches spaces+newlines, (.+) captures next line text');

    // Tab after # â€” \s includes tab
