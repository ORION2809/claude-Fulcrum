...
function executeValidatorSource(source) {
  const wrapperPath = path.join(
    validatorsDir,
    `.tmp-validator-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}.js`
  );

  fs.writeFileSync(wrapperPath, source, 'utf8');

  try {
    const stdout = execFileSync(nodeBinary, [wrapperPath], {
      encoding: 'utf8',
...
function runValidatorWithDir(validatorName, dirConstant, overridePath) {
  ...
  return executeValidatorSource(source);
}
...
function runValidator(validatorName) {
  const validatorPath = path.join(validatorsDir, `${validatorName}.js`);
  try {
    const stdout = execFileSync(nodeBinary, [validatorPath], {
      encoding: 'utf8',
      stdio: ['pipe', 'pipe', 'pipe'],
      timeout: 15000,
      cwd: repoRoot,
...
function writeCatalogFixture(testDir, options = {}) {
  const {
    ...
    structureLines = [
      'agents/          â€” 1 specialized subagents',
      'skills/          â€” 1 workflow skills and domain knowledge',
      'commands/        â€” 1 slash commands',
    ],
  } = options;
  ...
  fs.writeFileSync(readmePath, `Access to ${readmeCounts.agents} agents, ${readmeCounts.skills} skills, and ${readmeCounts.commands} commands.\n| Feature | Claude Code | Cursor IDE | Codex CLI | OpenCode |\n|---------|------------|------------|-----------|----------|\n| Agents | âœ… ${readmeCounts.agents} agents | Shared | Shared | 1 |\n| Commands | âœ… ${readmeCounts.commands} commands | Shared | Shared | 1 |\n| Skills | âœ… ${readmeCounts.skills} skills | Shared | Shared | 1 |\n`);
...
function runTests() {
  ...
  if (test('fails when README and AGENTS catalog counts drift', () => {
    ...
    const { readmePath, agentsPath } = writeCatalogFixture(testDir, {
      ...
      structureLines: [
        'agents/          â€” 99 specialized subagents',
        'skills/          â€” 99 workflow skills and domain knowledge',
        'commands/        â€” 99 slash commands',
      ],
    });
  ...
  if (test('captures ALL command references on a single line (multi-ref)', () => {
    ...
    const agentsDir = createTestDir();
    const skillsDir = createTestDir();
    // Line with two command references â€” both should be detected
  ...
  // â”€â”€ Round 27: hook validation edge cases â”€â”€
  ...
  // â”€â”€ Round 27: command validation edge cases â”€â”€
  ...
  // â”€â”€ Round 30: validate-commands skill warnings and workflow edge cases â”€â”€
  ...
  if (test('warns (not errors) when skill directory reference is not found', () => {
    ...
    // but the skill doesn't exist â€” should warn, not error
    ...
    // Skill directory references produce warnings, not errors â€” exit 0
  ...
  // â”€â”€ Round 32: empty frontmatter & edge cases â”€â”€
  ...
  if (test('rejects agent with no content between --- markers (Missing frontmatter)', () => {
    const testDir = createTestDir();
    // ---\n--- with no blank line â†’ regex doesn't match â†’ "Missing frontmatter"
  ...
  if (test('skips directory entries even if named with .md extension', () => {
    const testDir = createTestDir();
    // Create a directory named "tricky.md" â€” stat.isFile() should skip it
  ...
  // â”€â”€ Round 42: case sensitivity, space-before-colon, missing dirs, empty matchers â”€â”€
  ...
  if (test('handles space before colon in frontmatter key', () => {
    const testDir = createTestDir();
    // "model : sonnet" â€” space before colon. extractFrontmatter uses indexOf(':') + trim()
  ...
  if (test('flags agent path references when AGENTS_DIR does not exist', () => {
    const testDir = createTestDir();
    const skillsDir = createTestDir();
    // AGENTS_DIR points to non-existent path â†’ validAgents set stays empty
  ...
  // â”€â”€ Round 47: escape sequence and frontmatter edge cases â”€â”€
  ...
  if (test('validates inline JS with mixed escape sequences (newline + escaped quote)', () => {
    ...
    // After unescape chain: var a = "ok"\nconsole.log(a) (real newline) â€” valid JS
    fs.writeFileSync(hooksFile, JSON.stringify({
      hooks: {
        PreToolUse: [{ matcher: 'test', hooks: [{ type: 'command',
  ...
  if (test('rejects inline JS with syntax error after unescaping', () => {
    const testDir = createTestDir();
    const hooksFile = path.join(testDir, 'hooks.json');
    // After unescape this becomes: var x = { â€” missing closing brace
    fs.writeFileSync(hooksFile, JSON.stringify({
      hooks: {
        PreToolUse: [{ matcher: 'test', hooks: [{ type: 'command',
  ...
  if (test('silently ignores frontmatter line without colon', () => {
    const testDir = createTestDir();
    // Line "just some text" has no colon â€” should be skipped, not cause crash
    fs.writeFileSync(path.join(testDir, 'mixed.md'),
  ...
  // â”€â”€ Round 52: command inline backtick refs, workflow whitespace, code-only rules â”€â”€
  ...
  // â”€â”€ Round 57: readFileSync error path, statSync catch block, adjacent code blocks â”€â”€
  console.log('\nRound 57: validate-skills.js (SKILL.md is a directory â€” readFileSync error):');

  if (test('fails gracefully when SKILL.md is a directory instead of a file', () => {
    const testDir = createTestDir();
    const skillDir = path.join(testDir, 'dir-skill');
    fs.mkdirSync(skillDir);
    // Create SKILL.md as a DIRECTORY, not a file â€” existsSync returns true
  ...
  console.log('\nRound 57: validate-rules.js (broken symlink â€” statSync catch block):');
  ...
  if (test('reports error for broken symlink .md file in rules directory', () => {
    ...
    // Create a broken symlink (dangling â†’ target doesn't exist)
    ...
    try {
      ...
      console.log('    (skipped â€” symlinks not supported)');
      cleanupTestDir(testDir);
      return;
    }
  ...
  if (test('strips multiple adjacent code blocks before checking references', () => {
    ...
    const agentsDir = createTestDir();
    const skillsDir = createTestDir();
    // Two adjacent code blocks, each with broken refs â€” BOTH must be stripped
    ...
    assert.strictEqual(result.code, 0,
      'Both code blocks should be stripped â€” no broken refs reported');
  ...
  // â”€â”€ Round 58: readFileSync catch block, colonIdx edge case, command-as-object â”€â”€
  console.log('\nRound 58: validate-agents.js (unreadable agent file â€” readFileSync catch):');
  ...
  if (test('rejects agent when required field key has colon at position 0 (no key name)', () => {
    ...
    const result = runValidatorWithDir('validate-agents', 'AGENTS_DIR', testDir);
    assert.strictEqual(result.code, 1, 'Should fail â€” model field is missing (colon at idx 0 skipped)');
    assert.ok(result.stderr.includes('model'), 'Should report missing model field');
    cleanupTestDir(testDir);
  })) passed++; else failed++;

  console.log('\nRound 58: validate-hooks.js (command is a plain object â€” not string or array):');
  ...
  // â”€â”€ Round 63: object-format missing matcher, unreadable command file, empty commands dir â”€â”€
  ...
  if (test('passes on empty commands directory (no .md files)', () => {
    const testDir = createTestDir();
    // Only non-.md files â€” no .md files to validate
  ...
  // â”€â”€ Round 65: empty directories for rules and skills â”€â”€
  console.log('\nRound 65: validate-rules.js (empty directory â€” no .md files):');

  if (test('passes on rules directory with no .md files (Validated 0)', () => {
    const testDir = createTestDir();
    // Only non-.md files â€” readdirSync filter yields empty array
  ...
  console.log('\nRound 65: validate-skills.js (empty directory â€” no subdirectories):');

  if (test('passes on skills directory with only files, no subdirectories (Validated 0)', () => {
    const testDir = createTestDir();
    // Only files, no subdirectories â€” isDirectory filter yields empty array
  ...
  // â”€â”€ Round 70: validate-commands.js "would create:" line skip â”€â”€
  ...
  // â”€â”€ Round 72: validate-hooks.js async/timeout type validation â”€â”€
  ...
  // â”€â”€ Round 73: validate-commands.js skill directory statSync catch â”€â”€
  ...
  if (test('skips unreadable skill directory entries without error (broken symlink)', () => {
    ...
    // Broken symlink: target does not exist â€” statSync will throw ENOENT
    ...
    assert.strictEqual(result.code, 0,
      'Should pass â€” broken symlink in skills dir should be skipped silently');
  ...
  // â”€â”€ Round 76: validate-hooks.js invalid JSON in hooks.json â”€â”€
  ...
  // â”€â”€ Round 78: validate-hooks.js wrapped { hooks: { ... } } format â”€â”€
  console.log('\nRound 78: validate-hooks.js (wrapped hooks format):');

  if (test('validates wrapped format { hooks: { PreToolUse: [...] } }', () => {
    const testDir = createTestDir();
    const hooksFile = path.join(testDir, 'hooks.json');
    // The production hooks.json uses this wrapped format â€” { hooks: { ... } }
  ...
  // â”€â”€ Round 79: validate-commands.js warnings count suffix in output â”€â”€
  ...
  if (test('output includes (N warnings) suffix when skill references produce warnings', () => {
    ...
    // Each triggers a WARN (not error) â€” warnCount should be 2
  ...
  // â”€â”€ Round 80: validate-hooks.js legacy array format (lines 115-135) â”€â”€
  ...
  // â”€â”€ Round 82: Notification and SubagentStop event types â”€â”€
  ...
  // â”€â”€ Round 83: validate-agents whitespace-only field, validate-skills empty SKILL.md â”€â”€
  ...
  if (test('rejects agent with whitespace-only model field (trim guard)', () => {
    const testDir = createTestDir();
    // model has only whitespace â€” extractFrontmatter produces { model: '   ', tools: 'Read' }
    // The condition: typeof frontmatter[field] === 'string' && !frontmatter[field].trim()
    // evaluates to true for model â†’ "Missing required field: model"
