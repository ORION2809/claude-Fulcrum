...
async function runTests() {
  ...
  if (
    await asyncTest('slices user messages to last 10', async () => {
      ...
      // 15 user messages â€” should keep only last 10
      const lines = [];
      for (let i = 1; i <= 15; i++) {
  ...
  if (
    await asyncTest('slices tools to first 20', async () => {
      ...
      // 25 unique tools â€” should keep only first 20
  ...
  if (
    await asyncTest('slices files modified to first 30', async () => {
      ...
      // 35 unique files via Edit â€” should keep only first 30
  ...
  if (
    test('plugin.json does NOT have explicit hooks declaration', () => {
      ...
      // See: https://github.com/ORION2809/claude-Fulcrum/issues/103
  ...
  // â”€â”€â”€ evaluate-session.js tests â”€â”€â”€
  console.log('\nevaluate-session.js:');
  ...
  if (
    await asyncTest('skips short sessions (< 10 user messages)', async () => {
      ...
      // Only 3 user messages â€” below the default threshold of 10
  ...
  if (
    await asyncTest('evaluates long sessions (>= 10 user messages)', async () => {
      ...
      // 12 user messages â€” above the default threshold
      const lines = [];
      for (let i = 0; i < 12; i++) {
        lines.push(`{"type":"user","content":"message ${i}"}`);
      }
  ...
  if (
    await asyncTest('handles malformed stdin JSON (falls back to env var)', async () => {
      ...
      // No valid transcript path from either source â†’ exit 0
      assert.strictEqual(result.code, 0);
    })
  )
    passed++;
  ...
  // â”€â”€â”€ suggest-compact.js tests â”€â”€â”€
  console.log('\nsuggest-compact.js:');
  ...
  if (
    await asyncTest('suggests at periodic intervals after threshold', async () => {
      ...
      try {
        ...
        // (30 - 5) % 25 === 0 â†’ should trigger periodic suggestion
  ...
  // â”€â”€â”€ Round 20 bug fix tests â”€â”€â”€
  ...
  if (
    await asyncTest('stdout is exact byte match of stdin (no trailing newline)', async () => {
      ...
      const result = await runScript(path.join(scriptsDir, 'check-console-log.js'), stdinData);
      assert.strictEqual(result.code, 0);
      // stdout should be exactly the input â€” no extra newline appended
  ...
  // â”€â”€â”€ Round 23: Bug fixes & high-priority gap coverage â”€â”€â”€
  ...
  if (
    await asyncTest('respects min_session_length=0 from config (nullish coalescing)', async () => {
      ...
      const testDir = createTestDir();
      const transcriptPath = path.join(testDir, 'short.jsonl');
      // Only 2 user messages â€” normally below the default threshold of 10
  ...
  if (
    await asyncTest('handles invalid config JSON gracefully (uses defaults)', async () => {
      ...
      assert.strictEqual(result.code, 0);
      // Should log parse failure and fall back to default 10 â†’ 5 msgs too short
  ...
  if (
    await asyncTest('always updates session summary content on session end', async () => {
      ...
      const updated = fs.readFileSync(sessionFile, 'utf8');
      // Session summary should always be refreshed with current content 
  ...
  if (
    await asyncTest('handles transcript with only assistant messages (no user messages)', async () => {
      const testDir = createTestDir();
      const transcriptPath = path.join(testDir, 'transcript.jsonl');
      // Only assistant messages â€” no user messages
      ...
      // With no user messages, extractSessionSummary returns null â†’ blank template
      const claudeDir = path.join(testDir, '.claude', 'sessions');
      if (fs.existsSync(claudeDir)) {
        const files = fs.readdirSync(claudeDir).filter(f => f.endsWith('.tmp'));
        if (files.length > 0) {
  ...
  // â”€â”€â”€ Round 24: suggest-compact interval fix, fd fallback, session-start maxAge â”€â”€â”€
  ...
  if (
    await asyncTest('periodic intervals are consistent with non-25-divisible threshold', async () => {
      ...
      // (count - 13) % 25 === 0 â†’ 38-13=25, 63-13=50, etc.
  ...
  if (
    await asyncTest('does not suggest at old-style multiples that skip threshold offset', async () => {
      ...
      // New behavior: (50-13)%25 = 37%25 = 12 â†’ no suggestion
  ...
  if (
    await asyncTest('fd fallback: handles corrupted counter file gracefully', async () => {
      ...
      const counterFile = path.join(os.tmpdir(), `claude-tool-count-${sessionId}`);
      try {
        // Write non-numeric data to trigger parseInt â†’ NaN â†’ reset to 1
  ...
  if (
    await asyncTest('handles counter at exact 1000000 boundary', async () => {
      ...
      const counterFile = path.join(os.tmpdir(), `claude-tool-count-${sessionId}`);
      try {
        // 1000000 is the upper clamp boundary â€” should still increment
  ...
  // â”€â”€â”€ Round 25: post-edit-console-warn pass-through fix, check-console-log edge cases â”€â”€â”€
  ...
  // â”€â”€ Round 29: post-edit-format.js cwd fix and process.exit(0) consistency â”€â”€
  ...
  if (
    await asyncTest('detects console.log in commented-out code', async () => {
      ...
      const result = await runScript(path.join(scriptsDir, 'post-edit-console-warn.js'), stdinJson);
      assert.strictEqual(result.code, 0);
      // The regex /console\.log/ matches even in comments â€” this is intentional
  ...
  // â”€â”€ Round 32: post-edit-typecheck special characters & check-console-log â”€â”€
  ...
  if (
    await asyncTest('handles file path with shell metacharacters safely', async () => {
      ...
      // execFileSync prevents shell injection â€” just verify no crash
  ...
  // â”€â”€ Round 38: evaluate-session.js tilde expansion & missing config â”€â”€
  ...
  if (
    await asyncTest('expands ~ in learned_skills_path to home directory', async () => {
      const testDir = createTestDir();
      const transcriptPath = path.join(testDir, 'transcript.jsonl');
      // 1 user message â€” below threshold, but we only need to verify directory creation
      fs.writeFileSync(transcriptPath, '{"type":"user","content":"msg"}');

      const skillsDir = path.join(testDir, 'skills', 'continuous-learning');
      fs.mkdirSync(skillsDir, { recursive: true });
      const configPath = path.join(skillsDir, 'config.json');
      // Use ~ prefix â€” should expand to the HOME dir we set
      fs.writeFileSync(
        configPath,
        JSON.stringify({
      ...
      // The script creates the directory via ensureDir â€” check that it attempted to
  ...
  if (
    await asyncTest('does NOT expand ~ in middle of learned_skills_path', async () => {
      ...
      fs.mkdirSync(skillsDir, { recursive: true });
      const configPath = path.join(skillsDir, 'config.json');
      // Path with ~ in the middle â€” should NOT be expanded
      fs.writeFileSync(
        configPath,
        JSON.stringify({
  ...
  if (
    await asyncTest('uses defaults when config file does not exist', async () => {
      const testDir = createTestDir();
      const transcriptPath = path.join(testDir, 'transcript.jsonl');
      // 5 user messages â€” below default threshold of 10
      const lines = [];
      for (let i = 0; i < 5; i++) lines.push(`{"type":"user","content":"msg${i}"}`);
      fs.writeFileSync(transcriptPath, lines.join('\n'));
  ...
  // â”€â”€ Round 44: session-start.js empty session file â”€â”€
  ...
  // â”€â”€ Round 49: typecheck extension matching and session-end conditional sections â”€â”€
  ...
  if (
    await asyncTest('summary omits Files Modified and Tools Used when none found', async () => {
      ...
      const testDir = createTestDir();
      const transcriptPath = path.join(testDir, 'transcript.jsonl');
      // Only user messages â€” no tool_use entries at all
  ...
  // â”€â”€ Round 50: alias reporting, parallel compaction, graceful degradation â”€â”€
  ...
  // â”€â”€ Round 53: console-warn max matches and format non-existent file â”€â”€
  ...
  // â”€â”€ Round 55: maxAge boundary, multi-session injection, stdin overflow â”€â”€
  ...
  if (
    await asyncTest('handles stdin exceeding MAX_STDIN (1MB) gracefully', async () => {
      ...
      // Create stdin > 1MB: truncated JSON will be invalid â†’ falls back to env var
      ...
      try {
        ...
        // Truncated JSON â†’ JSON.parse throws â†’ falls back to env var â†’ creates session file
  ...
  // â”€â”€ Round 56: typecheck tsconfig walk-up, suggest-compact fallback path â”€â”€
  ...
  console.log('\nRound 56: suggest-compact.js (counter file as directory â€” fallback path):');

  if (
    await asyncTest('exits 0 when counter file path is occupied by a directory', async () => {
      const sessionId = `dirblock-${Date.now()}`;
      const counterFile = path.join(os.tmpdir(), `claude-tool-count-${sessionId}`);
      // Create a DIRECTORY at the counter file path â€” openSync('a+') will fail with EISDIR
  ...
  // â”€â”€ Round 59: session-start unreadable file, console-log stdin overflow, pre-compact write error â”€â”€
  console.log('\nRound 59: session-start.js (unreadable session file â€” readFile returns null):');
  ...
  if (
    await asyncTest('does not inject content when session file is unreadable', async () => {
      ...
      if (process.platform === 'win32' || (process.getuid && process.getuid() === 0)) {
        console.log('    (skipped â€” not supported on this platform)');
        return;
      }
      ...
      try {
        ...
        // readFile returns null for unreadable files â†’ content is null â†’ no injection
  ...
  console.log('\nRound 59: pre-compact.js (read-only session file â€” appendFile error):');
  ...
  if (
    await asyncTest('exits 0 when session file is read-only (appendFile fails)', async () => {
      ...
      try {
        ...
        // Should exit 0 â€” hooks must not block the user (catch at lines 45-47)
  ...
  // â”€â”€ Round 60: replaceInFile failure, console-warn stdin overflow, format missing tool_input â”€â”€
  console.log('\nRound 60: session-end.js (replaceInFile returns false â€” timestamp update warning):');
  ...
  if (
    await asyncTest('logs warning when existing session file lacks Last Updated field', async () => {
      ...
      // replaceInFile returns false â†’ line 166 logs warning about failed timestamp update
  ...
  console.log('\nRound 60: post-edit-console-warn.js (stdin exceeding 1MB â€” truncation):');
  ...
  if (
    await asyncTest('truncates stdin at 1MB limit and still passes through data', async () => {
      ...
      assert.strictEqual(result.code, 0, 'Should exit 0 even with oversized stdin');
      // Data should be truncated â€” stdout significantly less than input
      assert.ok(result.stdout.length < payload.length, `stdout (${result.stdout.length}) should be shorter than input (${payload.length})`);
  ...
  if (
    await asyncTest('skips formatting when JSON has no tool_input field', async () => {
      ...
      // input.tool_input?.file_path is undefined â†’ skips formatting â†’ passes through
  ...
  // â”€â”€ Round 64: post-edit-typecheck.js valid JSON without tool_input â”€â”€
  ...
  if (
    await asyncTest('skips typecheck when JSON has no tool_input field', async () => {
      ...
      assert.strictEqual(result.code, 0, 'Should exit 0 for JSON without tool_input');
      // input.tool_input?.file_path is undefined â†’ skips TS check â†’ passes through
      assert.strictEqual(result.stdout, stdinJson, 'Should pass through data unchanged when tool_input is absent');
    })
  )
    passed++;
  ...
  // â”€â”€ Round 66: session-end.js entry.role === 'user' fallback and nonexistent transcript â”€â”€
  ...
  // â”€â”€ Round 70: session-end.js entry.name / entry.input fallback in direct tool_use entries â”€â”€
  ...
  // â”€â”€ Round 71: session-start.js default source shows getSelectionPrompt â”€â”€
  console.log('\nRound 71: session-start.js (default source â€” selection prompt):');
  ...
  if (
    await asyncTest('shows selection prompt when no package manager preference found (default source)', async () => {
      ...
      // No package.json, no lock files, no package-manager.json â€” forces default source
  ...
  // â”€â”€ Round 74: session-start.js main().catch handler â”€â”€
  console.log('\nRound 74: session-start.js (main catch â€” unrecoverable error):');

  if (
    await asyncTest('session-start exits 0 with error message when HOME is non-directory', async () => {
      if (process.platform === 'win32') {
        console.log('    (skipped â€” /dev/null not available on Windows)');
        return;
      }
      // HOME=/dev/null makes ensureDir(sessionsDir) throw ENOTDIR,
      // which propagates to main().catch â€” the top-level error boundary
      const result = await runScript(path.join(scriptsDir, 'session-start.js'), '', {
        HOME: '/dev/null',
  ...
  // â”€â”€ Round 75: pre-compact.js main().catch handler â”€â”€
  console.log('\nRound 75: pre-compact.js (main catch â€” unrecoverable error):');
  ...
  // â”€â”€ Round 75: session-end.js main().catch handler â”€â”€
  console.log('\nRound 75: session-end.js (main catch â€” unrecoverable error):');
  ...
  if (
    await asyncTest('session-end exits 0 with error message when HOME is non-directory', async () => {
      ...
      // which propagates to runMain().catch â€” the top-level error boundary
      const result = await runScript(path.join(scriptsDir, 'session-end.js'), '{}', {
        HOME: '/dev/null',
        USERPROFILE: '/dev/null'
      });
  ...
  // â”€â”€ Round 76: evaluate-session.js main().catch handler â”€â”€
  console.log('\nRound 76: evaluate-session.js (main catch â€” unrecoverable error):');
  ...
  // â”€â”€ Round 76: suggest-compact.js main().catch handler â”€â”€
  console.log('\nRound 76: suggest-compact.js (main catch â€” double-failure):');
  ...
  // â”€â”€ Round 80: session-end.js entry.message?.role === 'user' third OR condition â”€â”€
  console.log('\nRound 80: session-end.js (entry.message.role user â€” third OR condition):');
  ...
  // â”€â”€ Round 81: suggest-compact threshold upper bound, session-end non-string content â”€â”€
  ...
  if (
    await asyncTest('COMPACT_THRESHOLD exceeding 10000 falls back to default 50', async () => {
      ...
      // Existing tests cover 0, negative, NaN â€” this covers the > 10000 boundary.
      const result = await runScript(path.join(scriptsDir, 'suggest-compact.js'), '', {
        COMPACT_THRESHOLD: '20000'
      });
      assert.strictEqual(result.code, 0, 'Should exit 0');
      // The script logs the threshold it chose â€” should fall back to 50
  ...
  if (
    await asyncTest('skips user messages with numeric content (non-string non-array branch)', async () => {
      ...
      const lines = [
        // Normal user message (string content) â€” should be included
        '{"type":"user","content":"Real user message"}',
        // User message with numeric content â€” exercises the else: '' branch
        '{"type":"user","content":42}',
        // User message with boolean content â€” also hits the else branch
        '{"type":"user","content":true}',
        // User message with object content (no .text) â€” also hits the else branch
  ...
  // â”€â”€ Round 82: tool_name OR fallback, template marker regex no-match â”€â”€
...
`;
  ...
  // â”€â”€ Round 87: post-edit-format.js and post-edit-typecheck.js stdin overflow (1MB) â”€â”€
  console.log('\nRound 87: post-edit-format.js (stdin exceeding 1MB â€” truncation):');

  if (
    await asyncTest('truncates stdin at 1MB limit and still passes through data (post-edit-format)', async () => {
      // Send 1.2MB of data â€” exceeds the 1MB MAX_STDIN limit (lines 14-22)
      const payload = 'x'.repeat(1024 * 1024 + 200000);
  ...
  console.log('\nRound 87: post-edit-typecheck.js (stdin exceeding 1MB â€” truncation):');

  if (
    await asyncTest('truncates stdin at 1MB limit and still passes through data (post-edit-typecheck)', async () => {
      // Send 1.2MB of data â€” exceeds the 1MB MAX_STDIN limit (lines 16-24)
      const payload = 'x'.repeat(1024 * 1024 + 200000);
  ...
  // â”€â”€ Round 89: post-edit-typecheck.js error detection path (relevantLines) â”€â”€
  ...
  // â”€â”€ Round 89: extractSessionSummary entry.name + entry.input fallback paths â”€â”€
  ...
  // â”€â”€ Round 90: readStdinJson timeout path (utils.js lines 215-229) â”€â”€
  ...
  if (
    await asyncTest('readStdinJson resolves with {} when stdin never closes (timeout fires, no data)', async () => {
      ...
      // Line 225: data.trim() is empty â†’ resolves with {}.
      ...
      return new Promise((resolve, reject) => {
        ...
        // Don't write anything or close stdin â€” force the timeout to fire
        let stdout = '';
  ...
  if (
    await asyncTest('readStdinJson resolves with {} when timeout fires with invalid partial JSON', async () => {
      ...
      // JSON.parse(data) throws â†’ catch at line 226 resolves with {}.
      const script = 'const u=require("./scripts/lib/utils");u.readStdinJson({timeoutMs:100}).then(d=>{process.stdout.write(JSON.stringify(d));process.exit(0)})';
      return new Promise((resolve, reject) => {
        const child = spawn('node', ['-e', script], {
          cwd: path.resolve(__dirname, '..', '..'),
        ...
        // Write partial invalid JSON but don't close stdin â€” timeout fires with unparseable data
  ...
  // â”€â”€ Round 94: session-end.js tools used but no files modified â”€â”€
  ...
  if (
    await asyncTest('session file includes Tools Used but omits Files Modified when only Read/Grep used', async () => {
      ...
      //   filesModified.length > 0 â†’ include "### Files Modified" section
      //   toolsUsed.length > 0 â†’ include "### Tools Used" section
