/**
 * Claude Fulcrum (Claude Fulcrum) Plugin Hooks for OpenCode
 *
 ...
 *
 * Hook Event Mapping:
 * - PreToolUse Ã¢â€ â€™ tool.execute.before
 * - PostToolUse Ã¢â€ â€™ tool.execute.after
 * - Stop Ã¢â€ â€™ session.idle / session.status
 * - SessionStart Ã¢â€ â€™ session.created
 * - SessionEnd Ã¢â€ â€™ session.deleted
 */
  ...
  return {
    ...
    "file.edited": async (event: { path: string }) => {
      ...
      if (hookEnabled("post:edit:format", ["standard", "strict"]) && event.path.match(/\.(ts|tsx|js|jsx)$/)) {
        try {
          ...
          log("info", `[GC] Formatted: ${event.path}`)
        } catch {
      ...
      if (hookEnabled("post:edit:console-warn", ["standard", "strict"]) && event.path.match(/\.(ts|tsx|js|jsx)$/)) {
        try {
          ...
          if (result.trim()) {
            ...
            log(
              "warn",
              `[GC] console.log found in ${event.path} (${lines} occurrence${lines > 1 ? "s" : ""})`
            )
          }
        } catch {
    ...
    "tool.execute.after": async (
      ...
      if (
        ...
        try {
          ...
          log("info", "[GC] TypeScript check passed")
        } catch (error: unknown) {
          const err = error as { stdout?: string }
          log("warn", "[GC] TypeScript errors detected:")
          if (err.stdout) {
      ...
      if (
        ...
        log("info", "[GC] PR created - check GitHub Actions status")
      }
    },
    ...
    "tool.execute.before": async (
      ...
      if (
        ...
        log(
          "info",
          "[GC] Remember to review changes before pushing: git diff origin/main...HEAD"
        )
      }
      ...
      if (
        ...
        if (
          ...
          log(
            "warn",
            `[GC] Creating ${filePath} - consider if this documentation is necessary`
          )
        }
      }
      ...
      if (hookEnabled("pre:bash:tmux-reminder", "strict") && input.tool === "bash") {
        ...
        if (
          ...
          log(
            "info",
            "[GC] Long-running command detected - consider using background execution"
          )
        }
      }
    },
    ...
    "session.created": async () => {
      ...
      log("info", `[GC] Session started - profile=${currentProfile}`)

      // Check for project-specific context files
      try {
        const hasClaudeMd = await $`test -f ${worktree}/CLAUDE.md && echo "yes"`.text()
        if (hasClaudeMd.trim() === "yes") {
          log("info", "[GC] Found CLAUDE.md - loading project context")
        }
      } catch {
        // No CLAUDE.md found
      }
    ...
    "session.idle": async () => {
      ...
      log("info", "[GC] Session idle - running console.log audit")
      ...
      if (totalConsoleLogCount > 0) {
        log(
          "warn",
          `[GC] Audit: ${totalConsoleLogCount} console.log statement(s) in ${filesWithConsoleLogs.length} file(s)`
        )
        filesWithConsoleLogs.forEach((f) =>
          log("warn", `  - ${f}`)
        )
        log("warn", "[GC] Remove console.log statements before committing")
      } else {
        log("info", "[GC] Audit passed: No console.log statements found")
      }
    ...
    "session.deleted": async () => {
      ...
      log("info", "[GC] Session ended - cleaning up")
      editedFiles.clear()
    },
    ...
    "todo.updated": async (event: { todos: Array<{ text: string; done: boolean }> }) => {
      ...
      if (total > 0) {
        log("info", `[GC] Progress: ${completed}/${total} tasks completed`)
      }
    },
    ...
    "experimental.session.compacting": async () => {
      const contextBlock = [
        ...
        "",
        "## Active Plugin: Claude Fulcrum v1.8.0",
    ...
    "permission.ask": async (event: { tool: string; args: unknown }) => {
      log("info", `[GC] Permission requested for: ${event.tool}`)
