...
# Claude Fulcrum Codex global regression sanity check.
# Validates that global ~/.codex state matches expected Claude Fulcrum integration.
...
if [[ -f "$AGENTS_FILE" ]]; then
  if rg -n '^# Claude Fulcrum \(ECC\) Ã¢â‚¬â€ Agent Instructions' "$AGENTS_FILE" >/dev/null 2>&1; then
    ok "AGENTS contains Claude Fulcrum root instructions"
  else
    fail "AGENTS missing Claude Fulcrum root instructions"
  fi

  if rg -n '^# Codex Supplement \(From Claude Fulcrum \.codex/AGENTS\.md\)' "$AGENTS_FILE" >/dev/null 2>&1; then
    ok "AGENTS contains Claude Fulcrum Codex supplement"
  else
    fail "AGENTS missing Claude Fulcrum Codex supplement"
  fi
fi
...
if [[ -d "$SKILLS_DIR" ]]; then
  ...
  if [[ "$missing_skills" -eq 0 ]]; then
    ok "All 16 Claude Fulcrum Codex skills are present"
  else
