 ...
 *
 * Codemap Generator for claude-fulcrum (Claude Fulcrum)
 *
...
function buildTree(dir: string, prefix = '', depth = 0): string {
  ...
  items.forEach((entry, i) => {
    ...
    const connector = isLast ? 'Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ ' : 'Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ ';
...
- [INDEX](./INDEX.md) Ã¢â‚¬â€ Full overview
...
function generateIndex(areas: Record<string, AreaInfo>, allFiles: string[]): string {
  ...
  const areaRows = Object.entries(areas)
    .map(([key, area]) => `| [${area.name}](./${key}.md) | ${area.files.length} files | ${area.directories.slice(0, 3).map((d) => `\`${d}\``).join(', ') || 'Ã¢â‚¬â€'} |`)
    .join('\n');

  const topLevelTree = buildTree(SRC_DIR);

  return `# Codebase Overview Ã¢â‚¬â€ CODEMAPS Index
...
- [Frontend](./frontend.md) Ã¢â‚¬â€ UI components, pages, hooks
- [Backend/API](./backend.md) Ã¢â‚¬â€ API routes, controllers, middleware
- [Database](./database.md) Ã¢â‚¬â€ Models, schemas, migrations
- [Integrations](./integrations.md) Ã¢â‚¬â€ External services & adapters
- [Workers](./workers.md) Ã¢â‚¬â€ Background jobs, queues, cron tasks
`;
}
