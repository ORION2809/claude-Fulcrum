...
function getHomeDir() {
  ...
  if (isWindows && typeof process.env.USERPROFILE === 'string' && process.env.USERPROFILE.trim().length > 0) {
  ...
  function searchDir(currentDir) {
    try {
      ...
      for (const entry of entries) {
        ...
        let isDirectoryEntry = false;

        try {
          isFileEntry = entry.isFile();
          isDirectoryEntry = entry.isDirectory();
        } catch {
          continue;
        }

        if (isFileEntry && regex.test(entry.name)) {
          let stats;
          try {
        ...
        } else if (isDirectoryEntry && recursive) {
          searchDir(fullPath);
        }
      }
    } catch (_err) {
