...
# Sync Claude Fulcrum assets into a local Codex CLI setup.
# - Backs up ~/.codex config and AGENTS.md
# - Replaces AGENTS.md with Claude Fulcrum AGENTS.md
...
generate_prompt_file() {
  ...
  local cmd_name="$3"
  {
    printf '# Claude Fulcrum Command Prompt: /%s\n\n' "$cmd_name"
    printf 'Source: %s\n\n' "$src"
    printf 'Use this prompt to run the Claude Fulcrum `%s` workflow.\n\n' "$cmd_name"
    awk '
      NR == 1 && $0 == "---" { fm = 1; next }
      fm == 1 && $0 == "---" { fm = 0; next }
      fm == 1 { next }
...
log "Replacing global AGENTS.md with Claude Fulcrum AGENTS + Codex supplement"
if [[ "$MODE" == "dry-run" ]]; then
...
else
  {
    cat "$AGENTS_ROOT_SRC"
    printf '\n\n---\n\n'
    printf '# Codex Supplement (From Claude Fulcrum .codex/AGENTS.md)\n\n'
    cat "$AGENTS_CODEX_SUPP_SRC"
  } > "$AGENTS_FILE"
fi

log "Syncing Claude Fulcrum Codex skills"
...
log "Generating prompt files from Claude Fulcrum commands"
...
# Claude Fulcrum Tool Prompt: run-tests
...
# Claude Fulcrum Tool Prompt: check-coverage
...
# Claude Fulcrum Tool Prompt: security-audit
...
# Claude Fulcrum Rule Pack: common (optional)

Apply Claude Fulcrum common engineering rules for this session. Use these files as the source of truth:
...
# Claude Fulcrum Rule Pack: typescript (optional)

Apply Claude Fulcrum common rules plus TypeScript-specific rules for this session.
...
# Claude Fulcrum Rule Pack: python (optional)

Apply Claude Fulcrum common rules plus Python-specific rules for this session.
...
# Claude Fulcrum Rule Pack: golang (optional)

Apply Claude Fulcrum common rules plus Go-specific rules for this session.
...
# Claude Fulcrum Rule Pack: swift (optional)

Apply Claude Fulcrum common rules plus Swift-specific rules for this session.
