...
- Never continuously check status after spawning a swarm Ã¢â‚¬â€ wait for results
- ALWAYS read a file before editing it
- NEVER commit secrets, credentials, or .env files

## File Organization

- NEVER save to root folder Ã¢â‚¬â€ use the directories below
...
- Never use MCP tools alone for execution Ã¢â‚¬â€ Task tool agents do the actual work
...
|------|---------|---------|------|-----------|
| **1** | Agent Booster (WASM) | <1ms | $0 | Simple transforms (varÃ¢â€ â€™const, add types, etc.) Ã¢â‚¬â€ **Skip LLM entirely** |
| **2** | Haiku | ~500ms | $0.0002 | Simple tasks, low complexity (<30%) |
| **3** | Sonnet/Opus | 2-5s | $0.003-0.015 | Complex reasoning, architecture, security (>30%) |

- Always check for `[AGENT_BOOSTER_AVAILABLE]` or `[TASK_MODEL_RECOMMENDATION]` before spawning agents
- Use Edit tool directly when `[AGENT_BOOSTER_AVAILABLE]` Ã¢â‚¬â€ intent types: `var-to-const`, `add-types`, `add-error-handling`, `async-await`, `add-logging`, `remove-console`
...
This repository uses **dual-mode orchestration** to run Claude Code (Ã°Å¸â€Âµ) and OpenAI Codex (Ã°Å¸Å¸Â¢) workers in parallel with shared memory coordination. Both platforms collaborate on development tasks with cross-learning.
...
// Ã°Å¸â€Âµ Claude Code workers (architecture, security, testing)
...
// Ã°Å¸Å¸Â¢ Codex workers (implementation, optimization)
...
|----------|---------|----------|
| `feature` | Ã°Å¸â€Âµ Architect Ã¢â€ â€™ Ã°Å¸Å¸Â¢ Coder Ã¢â€ â€™ Ã°Å¸â€Âµ Tester Ã¢â€ â€™ Ã°Å¸Å¸Â¢ Reviewer | Full feature development |
| `security` | Ã°Å¸â€Âµ Analyst Ã¢â€ â€™ Ã°Å¸Å¸Â¢ Scanner Ã¢â€ â€™ Ã°Å¸â€Âµ Reporter | Security audit workflow |
| `refactor` | Ã°Å¸â€Âµ Architect Ã¢â€ â€™ Ã°Å¸Å¸Â¢ Refactorer Ã¢â€ â€™ Ã°Å¸â€Âµ Tester | Code modernization |
| `bugfix` | Ã°Å¸â€Âµ Researcher Ã¢â€ â€™ Ã°Å¸Å¸Â¢ Coder Ã¢â€ â€™ Ã°Å¸â€Âµ Tester | Bug investigation & fix |
...
```
Level 0: [Ã°Å¸â€Âµ Architect]           # No dependencies - runs first
Level 1: [Ã°Å¸Å¸Â¢ Coder, Ã°Å¸â€Âµ Tester]    # Depends on Architect
Level 2: [Ã°Å¸â€Âµ Reviewer]            # Depends on Coder + Tester
Level 3: [Ã°Å¸Å¸Â¢ Optimizer]           # Depends on Reviewer approval
```
...
|-----------|-------------------|--------|
| Architecture & Design | Ã°Å¸â€Âµ Claude | Strong reasoning, system thinking |
| Implementation | Ã°Å¸Å¸Â¢ Codex | Fast code generation |
| Security Review | Ã°Å¸â€Âµ Claude | Careful analysis, threat modeling |
| Performance Optimization | Ã°Å¸Å¸Â¢ Codex | Code-level optimizations |
| Testing Strategy | Ã°Å¸â€Âµ Claude | Coverage analysis, edge cases |
| Refactoring | Ã°Å¸Å¸Â¢ Codex | Bulk code transformations |
...
- `InputValidator` Ã¢â‚¬â€ Zod-based validation at boundaries
- `PathValidator` Ã¢â‚¬â€ Path traversal prevention
- `SafeExecutor` Ã¢â‚¬â€ Command injection protection
- `PasswordHasher` Ã¢â‚¬â€ bcrypt hashing
- `TokenGenerator` Ã¢â‚¬â€ Secure token generation
...
## Intelligence System (GC-Vector)

V3 includes the GC-Vector Intelligence System:
...
1. **RETRIEVE** Ã¢â‚¬â€ Fetch relevant patterns via HNSW
2. **JUDGE** Ã¢â‚¬â€ Evaluate with verdicts (success/failure)
3. **DISTILL** Ã¢â‚¬â€ Extract key learnings via LoRA
4. **CONSOLIDATE** Ã¢â‚¬â€ Prevent catastrophic forgetting via EWC++
...
- **Neural substrate**: Integration with GC-Vector

## Hive-Mind Consensus

### Topologies
- `hierarchical` Ã¢â‚¬â€ Queen controls workers directly
- `mesh` Ã¢â‚¬â€ Fully connected peer network
- `hierarchical-mesh` Ã¢â‚¬â€ Hybrid (recommended)
- `adaptive` Ã¢â‚¬â€ Dynamic based on load

### Consensus Strategies
- `byzantine` Ã¢â‚¬â€ BFT (tolerates f < n/3 faulty)
- `raft` Ã¢â‚¬â€ Leader-based (tolerates f < n/2)
- `gossip` Ã¢â‚¬â€ Epidemic for eventual consistency
- `crdt` Ã¢â‚¬â€ Conflict-free replicated data types
- `quorum` Ã¢â‚¬â€ Configurable quorum-based
...
- Keep MCP for coordination strategy only Ã¢â‚¬â€ use Claude Code's Task tool for real execution

## Publishing to npm

### Publishing Rules

- MUST publish ALL THREE packages when publishing CLI changes: `@claude-flow/cli`, `claude-flow`, AND `Claude Fulcrum`
- MUST update ALL dist-tags for ALL THREE packages after publishing
- Publish order: `@claude-flow/cli` first, then `claude-flow` (umbrella), then `Claude Fulcrum` (alias umbrella)
...
# STEP 4: Publish Claude Fulcrum umbrella (CRITICAL - DON'T FORGET!)
cd /workspaces/claude-flow/Claude Fulcrum
npm version 3.0.0-alpha.XXX --no-git-tag-version
npm publish --tag alpha
npm dist-tag add Claude Fulcrum@3.0.0-alpha.XXX latest
```
...
npm view Claude Fulcrum dist-tags --json
...
| `claude-flow` | `alpha` | `npx claude-flow@alpha` Ã¢â‚¬â€ EASY TO FORGET |
| `claude-flow` | `latest` | `npx claude-flow@latest` |
| `claude-flow` | `v3alpha` | `npx claude-flow@v3alpha` |
| `Claude Fulcrum` | `alpha` | `npx Claude Fulcrum@alpha` Ã¢â‚¬â€ EASY TO FORGET |
| `Claude Fulcrum` | `latest` | `npx Claude Fulcrum@latest` |

- Never forget the `Claude Fulcrum` package Ã¢â‚¬â€ it's a thin wrapper users run via `npx Claude Fulcrum@alpha`
- Never forget the umbrella `alpha` tag Ã¢â‚¬â€ users run `npx claude-flow@alpha`
- `Claude Fulcrum` source is in `/Claude Fulcrum/` Ã¢â‚¬â€ it depends on `@claude-flow/cli`
...
- Documentation: https://github.com/ORION2809/claude-Fulcrum
- Issues: https://github.com/ORION2809/claude-Fulcrum/issues
