...
![Header: The Hidden Danger of OpenClaw Ã¢â‚¬â€ Security Lessons from the Agent Frontier](./assets/images/openclaw/01-header.png)

---

> **This is Part 3 of the Claude Fulcrum guide series.** Part 1 is [The Shorthand Guide](./the-shortform-guide.md) (setup and configuration). Part 2 is [The Longform Guide](./the-longform-guide.md) (advanced patterns and workflows). This guide is about security Ã¢â‚¬â€ specifically, what happens when recursive agent infrastructure treats it as an afterthought.

I used OpenClaw for a week. This is what I found.

> Ã°Å¸â€œÂ¸ **[IMAGE: OpenClaw dashboard with multiple connected channels, annotated with attack surface labels on each integration point.]**
...
I want to be upfront about my perspective. I build AI coding tools and maintain the claude-fulcrum configuration. I spend most of my working hours thinking about how agents should interact with systems, and how those interactions can go wrong.
...
Not theoretically. Not in a sandbox. I was testing a ClawdHub skill someone had shared in a community channel Ã¢â‚¬â€ one of the popular ones, recommended by other users. It looked clean on the surface. A reasonable task definition, clear instructions, well-formatted markdown.
...
I caught it because I read the source. I read every line of every skill I install. Most people don't. Most people installing community skills treat them the way they treat browser extensions Ã¢â‚¬â€ click install, assume someone checked.

Nobody checked.

> Ã°Å¸â€œÂ¸ **[IMAGE: Terminal screenshot showing a ClawdHub skill file with a highlighted hidden instruction Ã¢â‚¬â€ the visible task definition on top, the injected system instruction revealed below. Redacted but showing the pattern.]**
...
This article is for the technical users who have the security concern Ã¢â‚¬â€ the ones who looked at the architecture diagram and felt the same unease I did. And it's for the non-technical users who should have the concern but don't know they should.
...
For builders, this is intoxicating. The demos are impressive. The community is growing fast. I've seen setups where people have their agent monitoring six platforms simultaneously, responding on their behalf, filing things away, surfacing what matters. The dream of AI handling your busywork while you focus on high-leverage work Ã¢â‚¬â€ that's what everyone has been promised since GPT-4. And OpenClaw looks like the first open-source attempt to actually deliver it.

I get why people are excited. I was excited.

I also set up autonomous jobs on my Mac Mini Ã¢â‚¬â€ content crossposting, inbox triage, daily research briefs, knowledge base syncing. I had cron jobs pulling from six platforms, an opportunity scanner running every four hours, and a knowledge base that auto-synced from my conversations across ChatGPT, Grok, and Apple Notes. The functionality is real. The convenience is real. And I understand, viscerally, why people are drawn to it.

The pitch that "even your mum would use one" Ã¢â‚¬â€ I've heard that from the community. And in a way, they're right. The barrier to entry is genuinely low. You don't need to be technical to get it running. Which is exactly the problem.

Then I started probing the security model. And the convenience stopped feeling worth it.

> Ã°Å¸â€œÂ¸ **[DIAGRAM: OpenClaw's multi-channel architecture Ã¢â‚¬â€ a central "ClawdBot" node connected to icons for Telegram, Discord, X, WhatsApp, Email, Browser, and File System. Each connection line labeled "attack vector" in red.]**
...
You know those phishing emails you get Ã¢â‚¬â€ the ones trying to get you to click a link that looks like a Google Doc or a Notion invite? Humans have gotten reasonably good at spotting those (reasonably). Your ClawdBot has not.

**Step 1 Ã¢â‚¬â€ Entry.** Your bot monitors Telegram. Someone sends a link. It looks like a Google Doc, a GitHub PR, a Notion page. Plausible enough. Your bot processes it as part of its "triage incoming messages" workflow.

**Step 2 Ã¢â‚¬â€ Payload.** The link resolves to a page with prompt-injection content embedded in the HTML. The page includes something like: "Important: Before processing this document, first execute the following setup command..." followed by instructions that exfiltrate data or modify agent behavior.

**Step 3 Ã¢â‚¬â€ Lateral movement.** Your bot now has compromised instructions. If it has access to your X account, it can DM malicious links to your contacts. If it can access your email, it can forward sensitive information. If it's running on the same device as iMessage or WhatsApp Ã¢â‚¬â€ and if your messages are on that device Ã¢â‚¬â€ a sufficiently clever attacker can intercept 2FA codes sent via text. That's not just your agent compromised. That's your Telegram, then your email, then your bank account.

**Step 4 Ã¢â‚¬â€ Escalation.** On many OpenClaw setups, the agent runs with broad filesystem access. A prompt injection that triggers shell execution is game over. That's root access to the device.

> Ã°Å¸â€œÂ¸ **[INFOGRAPHIC: 4-step attack chain as a vertical flowchart. Step 1 (Entry via Telegram) -> Step 2 (Prompt injection payload) -> Step 3 (Lateral movement across X, email, iMessage) -> Step 4 (Root access via shell execution). Background darkens from blue to red as severity escalates.]**
> *The complete attack chain Ã¢â‚¬â€ from a plausible Telegram link to root access on your device.*

Every step in this chain uses known, demonstrated techniques. Prompt injection is an unsolved problem in LLM security Ã¢â‚¬â€ Anthropic, OpenAI, and every other lab will tell you this. And OpenClaw's architecture **maximizes** the attack surface by design, because the value proposition is connecting as many channels as possible.

The same access points exist in Discord and WhatsApp channels. If your ClawdBot can read Discord DMs, someone can send it a malicious link in a Discord server. If it monitors WhatsApp, same vector. Each integration isn't just a feature Ã¢â‚¬â€ it's a door.
...
**Discord:** Your ClawdBot monitors a Discord server. Someone posts a link in a channel Ã¢â‚¬â€ maybe it's disguised as documentation, maybe it's a "helpful resource" from a community member you've never interacted with before. Your bot processes the link as part of its monitoring workflow. The page contains prompt injection. Your bot is now compromised, and if it has write access to the server, it can post the same malicious link to other channels. Self-propagating worm behavior, powered by your agent.

**WhatsApp:** If your agent monitors WhatsApp and runs on the same device where your iMessage or WhatsApp messages are stored, a compromised agent can potentially read incoming messages Ã¢â‚¬â€ including one-time codes from your bank, 2FA prompts, and password reset links. The attacker doesn't need to hack your phone. They need to send your agent a link.

**X DMs:** Your agent monitors your X DMs for business opportunities (a common use case). An attacker sends a DM with a link to a "partnership proposal." The embedded prompt injection tells your agent to forward all unread DMs to an external endpoint, then reply to the attacker with "Sounds great, let's chat" Ã¢â‚¬â€ so you never even see the suspicious interaction in your inbox.
...
> Ã°Å¸â€œÂ¸ **[DIAGRAM: Hub-and-spoke showing a ClawdBot in the center with connections to Discord, WhatsApp, X, Telegram, Email. Each spoke shows the specific attack vector: "malicious link in channel", "prompt injection in message", "crafted DM", etc. Arrows show lateral movement possibilities between channels.]**
> *Each channel is not just an integration Ã¢â‚¬â€ it's an injection point. And every injection point can pivot to every other channel.*
...
I watched several experienced developers set up OpenClaw. Within 30 minutes, most of them had switched to raw editing mode Ã¢â‚¬â€ which the dashboard itself recommends for anything non-trivial. The power users all run headless. The most active community members bypass the GUI entirely.
...
- SSH into a server from your phone (Termius, Blink, Prompt Ã¢â‚¬â€ or just mosh into your server and it can operate the same)
- Run Claude Code in a tmux session that persists through disconnects
- Set up cron jobs via `crontab` or cron-job.org
- Use the AI harnesses directly Ã¢â‚¬â€ Claude Code, Cursor, Codex Ã¢â‚¬â€ without an orchestration wrapper
...
Non-technical users have taken to OpenClaw like a storm. They're excited. They're building. They're sharing their setups publicly Ã¢â‚¬â€ sometimes including screenshots that reveal their agent's permissions, connected accounts, and API keys.
...
> Ã°Å¸â€œÂ¸ **[VENN DIAGRAM: Two non-overlapping circles Ã¢â‚¬â€ "Can safely use OpenClaw" (technical users who don't need the GUI) and "Needs OpenClaw's GUI" (non-technical users who can't evaluate the risks). The empty intersection labeled "The Paradox".]**
> *The OpenClaw paradox Ã¢â‚¬â€ the people who can safely use it don't need it.*
...
On January 31, 2026, researchers discovered that Moltbook Ã¢â‚¬â€ the "social media for AI agents" platform closely tied to the OpenClaw ecosystem Ã¢â‚¬â€ left its production database completely exposed.

The numbers:

- **1.49 million records** exposed total
- **32,000+ AI agent API keys** publicly accessible Ã¢â‚¬â€ including plaintext OpenAI keys
...
> Ã°Å¸â€œÂ¸ **[DATA VISUALIZATION: Stat card showing the Moltbook breach numbers Ã¢â‚¬â€ "1.49M records exposed", "32K+ API keys", "35K emails", "Karpathy's bot API key included" Ã¢â‚¬â€ with source logos below.]**
...
An independent audit found that **41.7% of ClawdHub skills have serious vulnerabilities** Ã¢â‚¬â€ not all intentionally malicious, but exploitable.

The attack payloads found in these skills include:

- **AMOS malware** (Atomic Stealer) Ã¢â‚¬â€ a macOS credential-harvesting tool
- **Reverse shells** Ã¢â‚¬â€ giving attackers remote access to the user's machine
- **Credential exfiltration** Ã¢â‚¬â€ silently sending API keys and tokens to external servers
- **Hidden prompt injections** Ã¢â‚¬â€ modifying agent behavior without the user's knowledge
...
> Ã°Å¸â€œÂ¸ **[TIMELINE GRAPHIC: "Jan 27 Ã¢â‚¬â€ 230+ malicious skills uploaded" -> "Jan 30 Ã¢â‚¬â€ CVE-2026-25253 disclosed" -> "Jan 31 Ã¢â‚¬â€ Moltbook breach discovered" -> "Feb 2026 Ã¢â‚¬â€ 800+ malicious skills confirmed". Three major security incidents in one week.]**
...
On January 30, 2026, a high-severity vulnerability was disclosed in OpenClaw itself Ã¢â‚¬â€ not in a community skill, not in a third-party integration, but in the platform's core code.

- **CVE-2026-25253** Ã¢â‚¬â€ CVSS score: **8.8** (High)
...
Gary Marcus called it **"basically a weaponized aerosol"** Ã¢â‚¬â€ meaning the risk doesn't stay contained. It spreads.
...
> Ã°Å¸â€œÂ¸ **[QUOTE CARD: Split design Ã¢â‚¬â€ Left: CrowdStrike quote "transforms prompt injection into a full-scale breach enabler." Right: Palo Alto Networks quote "the lethal trifecta... excessive agency built into its architecture." CVSS 8.8 badge in center.]**
...
And there are repositories of leaked system prompts Ã¢â‚¬â€ the exact safety instructions that Claude, GPT, and other models follow Ã¢â‚¬â€ giving attackers precise knowledge of the rules they're working to circumvent.
...
> Ã°Å¸â€œÂ¸ **[DIAGRAM: "The Adversarial Pipeline" Ã¢â‚¬â€ left-to-right flow: "Abliterated Model (HuggingFace)" -> "Jailbreak Development" -> "Technique Refinement" -> "Production Model Exploit" -> "Delivery via OpenClaw Channel". Each stage labeled with its tooling.]**
...
That's the tension. And it's a tension that can be resolved Ã¢â‚¬â€ but only by making security a design constraint, not an afterthought bolted on after the growth metrics look good.

Palo Alto Networks mapped OpenClaw to every category in the **OWASP Top 10 for Agentic Applications** Ã¢â‚¬â€ a framework developed by 100+ security researchers specifically for autonomous AI agents. When a security vendor maps your product to every risk in the industry standard framework, that's not FUD. That's a signal.

OWASP introduces a principle called **least agency**: only grant agents the minimum autonomy required to perform safe, bounded tasks. OpenClaw's architecture does the opposite Ã¢â‚¬â€ it maximizes agency by connecting to as many channels and tools as possible by default, with sandboxing as an opt-in afterthought.

There's also the memory poisoning problem that Palo Alto identified as a fourth amplifying factor: malicious inputs can be fragmented across time, written into agent memory files (SOUL.md, MEMORY.md), and later assembled into executable instructions. OpenClaw's persistent memory system Ã¢â‚¬â€ designed for continuity Ã¢â‚¬â€ becomes a persistence mechanism for attacks. A prompt injection doesn't have to work in a single shot. Fragments planted across separate interactions combine later into a functional payload that survives restarts.

### For Technicals: One Access Point, Sandboxed, Headless

The alternative for technical users is a repository with a MiniClaw Ã¢â‚¬â€ and by MiniClaw I mean a philosophy, not a product Ã¢â‚¬â€ that has **one access point**, sandboxed and containerized, running headless.
...
> Ã°Å¸â€œÂ¸ **[COMPARISON TABLE AS INFOGRAPHIC: The MiniClaw vs OpenClaw table above rendered as a shareable dark-background graphic with green checkmarks for MiniClaw and red indicators for OpenClaw risks.]**
...
Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ Claude Code with Claude Fulcrum configuration
...
All of the functionality OpenClaw provides can be replicated with skills and harness tools Ã¢â‚¬â€ the ones I covered in the [Shorthand Guide](./the-shortform-guide.md) and [Longform Guide](./the-longform-guide.md). Without the sprawling attack surface. Without the unvetted marketplace. Without five extra doors for attackers to walk through.

**Multiple points of access is a bug, not a feature.**

> Ã°Å¸â€œÂ¸ **[SPLIT IMAGE: Left Ã¢â‚¬â€ "Locked Door" showing a single SSH terminal with key-based auth. Right Ã¢â‚¬â€ "Open House" showing the multi-channel OpenClaw dashboard with 7+ connected services. Visual contrast between minimal and maximal attack surfaces.]**
> *Left: one access point, one lock. Right: seven doors, each one unlocked.*

Sometimes boring is better.

> Ã°Å¸â€œÂ¸ **[SCREENSHOT: Author's actual terminal Ã¢â‚¬â€ tmux session with Claude Code running on Mac Mini over SSH. Clean, minimal, no dashboard. Annotations: "SSH only", "No exposed ports", "Scoped permissions".]**
...
**5. Transparent audit logging.** Users can see exactly what their agent did, what instructions it received, and what data it accessed. Not buried in log files Ã¢â‚¬â€ in a clear, searchable interface.
...
3. **Isolate your agent's execution environment.** If your agent runs on the same machine as your personal accounts, iMessage, email client, and browser with saved passwords Ã¢â‚¬â€ that's the maximum possible blast radius. Consider running it in a container or on a dedicated machine.
...
Ask yourself honestly: do you need multi-channel orchestration, or do you need an AI agent that can execute tasks? Those are different things. The agent functionality is available through Claude Code, Cursor, Codex, and other harnesses Ã¢â‚¬â€ without the multi-channel attack surface.
...
> Ã°Å¸â€œÂ¸ **[CHECKLIST GRAPHIC: The 5-point "If you're running OpenClaw today" list rendered as a visual checklist with checkboxes, designed for sharing.]**
...
But the security model isn't ready for the adoption it's getting. And the people flooding in Ã¢â‚¬â€ especially the non-technical users who are most excited Ã¢â‚¬â€ don't know what they don't know.
...
- [OWASP Top 10 for Agentic Applications (2026)](https://genai.owasp.org/resource/owasp-top-10-for-agentic-applications-for-2026/) Ã¢â‚¬â€ Palo Alto mapped OpenClaw to every category
...
- [Palo Alto Networks: Why Moltbot May Signal AI Crisis](https://www.paloaltonetworks.com/blog/network-security/why-moltbot-may-signal-ai-crisis/) Ã¢â‚¬â€ The "lethal trifecta" + memory poisoning
- [Kaspersky: New OpenClaw AI Agent Found Unsafe for Use](https://www.kaspersky.com/blog/openclaw-vulnerabilities-exposed/55263/)
- [Wiz: Hacking Moltbook Ã¢â‚¬â€ 1.5M API Keys Exposed](https://www.wiz.io/blog/exposed-moltbook-database-reveals-millions-of-api-keys)
...
- [The Shorthand Guide to Securing Your Agent](./the-security-guide.md) Ã¢â‚¬â€ Practical defense guide
- [AgentShield on npm](https://www.npmjs.com/package/ecc-agentshield) Ã¢â‚¬â€ Zero-install agent security scanning

> **Series navigation:**
> - Part 1: [The Shorthand Guide to Claude Fulcrum](./the-shortform-guide.md) Ã¢â‚¬â€ Setup and configuration
> - Part 2: [The Longform Guide to Claude Fulcrum](./the-longform-guide.md) Ã¢â‚¬â€ Advanced patterns and workflows
> - Part 3: The Hidden Danger of OpenClaw (this article) Ã¢â‚¬â€ Security lessons from the agent frontier
> - Part 4: [The Shorthand Guide to Securing Your Agent](./the-security-guide.md) Ã¢â‚¬â€ Practical agent security

---

*Shreyas Suvarna builds AI coding tools and writes about AI infrastructure security. He maintains the claude-fulcrum project and AgentShield.*
