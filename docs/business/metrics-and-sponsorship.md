...
```bash
# Weekly downloads
curl -s https://api.npmjs.org/downloads/point/last-week/claude-fulcrum
curl -s https://api.npmjs.org/downloads/point/last-week/ecc-agentshield

# Last 30 days
curl -s https://api.npmjs.org/downloads/point/last-month/claude-fulcrum
...
```bash
gh api repos/ORION2809/claude-Fulcrum \
...
```bash
gh api repos/ORION2809/claude-Fulcrum/traffic/views
gh api repos/ORION2809/claude-Fulcrum/traffic/clones
```
...
- [Claude Fulcrum Tools Marketplace](https://github.com/marketplace/ecc-tools)
...
> Claude Fulcrum is now positioned as an agent harness performance system, not a config repo.  
